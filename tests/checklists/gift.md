...
## ⚡ 2026-08-04 P1 批次落地说明（增量）

> 本节记录 P1 修复后与下文历史清单的口径差异，历史条目保留作审计线索，**以本节为准**。

1. **权限口径已切换为 ORG_SHARED（家庭组共享）**：record/person/event 的越权判定从"仅本人"改为"同机构互通、跨机构拒绝"，异常消息统一为"无权访问其他机构的X"。`GiftRecordInfoServiceIT` 已按新口径改写（含同组放行正例），下文 H2/H3/H4/H7 等"跨用户拒绝"条目按跨机构理解。
2. **`returned_flag` 与回礼流水自动联动**：RETURN 记录新增/修改/删除后由 `syncReturnedFlag` 按 `sum(RETURN.amount) >= RECEIVE.amount` 重算，覆盖手动误标；`markReturned` 保留为手动标记入口且幂等（I11 行为已确认：重复 mark 直接返回 true）。
3. **事务边界补齐**：record 的增/改/删/markReturned、person 的增/改、`listOrgMemberOptions`（懒建档插库）均已挂 `@Transactional(rollbackFor = Exception.class)`。
4. **数据权限缺口收口**：`sumReturnAmountByRelatedRecordId` 已挂 `@DataPermission`（无别名，按表名限定 `gift_record_info_t.org_id`）；`DataPermissionHandlerImpl` 对带 `alias="r"` 的注解按别名限定列，断言见 `GiftRecordDataPermissionPageIT`。
5. **导出契约**：`POST /export` 在 controller 层兜底声明 `Content-Disposition: attachment`，服务层用带时间戳文件名覆盖；PC 端按响应头解析文件名下载。
6. **`listOrgMemberOptions` 性能**：已建档成员一次 IN 批量查询，仅缺档成员逐个 Feign 取展示名（懒建档语义保留）。
7. **统计口径**：`GiftRecordSummaryVo` 新增 `receiveCount/giveCount/returnCount`；移动端 analysis 页改调 `/gift-analysis/overview` 全量聚合，不再"取前 200 条本地计算"。
8. **person 汇总 `yearTotalAmount`**：仅统计当前自然年（payTime）记录。
9. **analysis 聚合 SQL 化（P1 第二批）**：`overview/summary` 走 `sumDirectionAgg`（GROUP BY direction）、`trend` 走 `sumTrendAgg`（DATE_FORMAT 分组，月 `%Y-%m`/年 `%Y`）、`relation-distribution` 走 `countRelationDistribution`（person 表 GROUP BY relation_type，空值归 OTHER），三个新 mapper 方法均挂 `@DataPermission(ORG_SHARED)`；不再全量拉记录内存聚合。
10. **analysis 筛选参数**：`GET /gift-analysis/overview` 支持 `direction`，`GET /trend` 支持 `period`（month/year）+`direction`，方向白名单校验（非法值按全部）；PC analysis 页 period/analysisType 已透传后端并联动刷新。
11. **PC ID 契约锁**：`normalizeGiftIds/normalizeGiftResponse` 抽至 `src/views/finance/gift/api/normalize.ts` 纯模块，`tests/unit/giftNormalize.test.mts` 用 Node 22 内置 test runner 锁行为（`npm run test:unit`；vitest 4 与 Vite 8 Rolldown 不兼容故弃用）。
12. **移动端快记 picker**：事由/送礼人/收礼人由手输 ID 改为 van-picker 选择（testid：`gift-record-event-picker/gift-record-giver-picker/gift-record-receiver-picker/gift-record-picker`），选项打开面板时懒加载。

## ⚡ 2026-08-04 P2 批次落地说明（增量）

13. **移动端回礼原记录选择器**：`relatedRecordId` 改为 picker（testid `gift-record-related-picker`），选项来源 `record/page` 取 `direction=RECEIVE` 前 50 条并过滤 `returnedFlag === 1`（已回礼的不再作为回礼目标），保存或手动 mark-returned 后缓存失效重取；该字段在 RETURN 方向下为必填（表单校验）。
14. **移动端事由时间选择补时分**：`giftEventDetail` 由单个 `van-date-picker`（时分列不渲染、恒为 00:00 的历史缺陷）改为 `van-picker-group` 两步式（日期 + 时间，testid `gift-event-time-picker`）；`components.d.ts` 同步补 `VanPickerGroup/VanTimePicker`。
15. **PC 隐藏筛选项开放**：record 页展开区新增「关联事由 / 送礼人 / 收礼人 / 回礼状态」（testid `gift-record-filter-event|giver|receiver|return-status`，前三项复用现成 `gift-event-picker`、`gift-contact-picker` 且关闭新建入口）；person 页新增「人员范围」（testid `gift-person-filter-scope`，CONTACT/ORG_MEMBER，对应 mapper 的 `bind_user_id IS (NOT) NULL` 分支）。选项常量集中在 `config.ts` 的 `giftReturnStatusOptions`/`giftPersonScopeOptions`。
...
| 后端 API 前缀   | `/gift-record-info-t` `/gift-person-info-t` `/gift-event-info-t` `/gift-analysis`                                  |
| 数据库表        | `gift_record_info_t` `gift_person_info_t` `gift_event_info_t`（relation 表已于 2026-08-04 下线）                   |
...
| H13 | ~~`GiftRelationInfoTServiceImp.delete` 的 String ids 转换~~（relation 子域已于 2026-08-04 整体移除，不再适用）                  | —                                                              |
...
### 2.4 `GiftRelationInfoT`（人际关系）— 已移除

> relation 子域（表 + 后端代码）已于 2026-08-04 下线：前端从未接入，关系语义由
> `gift_person_info_t.relation_type` + `gift_person_relation_option_t` 承载。本节不再适用。
...
#### 5.1.4 `GiftRelationInfoTServiceImpTest` — 不再适用（relation 子域已移除）
