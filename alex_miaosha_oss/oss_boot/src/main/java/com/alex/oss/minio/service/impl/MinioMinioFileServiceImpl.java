...
public class MinioMinioFileServiceImpl implements MinioFileService {
    ...
    public FileInfoVo uploadFile(MultipartFile file, String type, Boolean isThumbnail, Boolean isNormal) throws Exception {
        ...
        if (isThumbnail) {
            Map<String, String> stringStringMap = minioTemplate.thumbnail(bucketName, filename, file.getInputStream(), file.getContentType());
            fileVo.setThumbnailUrl(stringStringMap.get("url"));
        }
        if (isNormal) {
            Map<String, String> upload = minioTemplate.upload(bucketName, filename, file.getInputStream(), file.getContentType());
