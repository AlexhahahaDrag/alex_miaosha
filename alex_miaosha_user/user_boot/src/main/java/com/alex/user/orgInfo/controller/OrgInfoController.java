...
@RequestMapping("${api.version:/api/v1}/org-info")
public class OrgInfoController {

    private final OrgInfoService orgInfoService;

    @LogRestRequest(apiName = "获取机构表分页")
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(value = "查询条件", name = "orgInfoVo", dataTypeClass = OrgInfoVo.class)}
    )
    ...
    @LogRestRequest(apiName = "删除机构表")
    @ApiOperationSupport(order = 50, author = "alex")
    @ApiOperation(value = "删除机构表", notes = "删除机构表", response = Result.class)
    @DeleteMapping
