...
/**
 * RBAC-BE-ORG-004: 机构删除守卫——存在下级机构或仍绑定有效用户时必须拒删，
 * 对齐 {@link MenuDeleteGuardTest} 的用例结构。
 */
@ExtendWith(MockitoExtension.class)
public class OrgDeleteGuardTest {
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(service, "baseMapper", orgInfoMapper);
    }
    ...
    @Test
    void deleteOrgInfo_deniedWhenOutOfOwnershipScope_beforeChildOrBoundUserGuards() {
        when(userUtils.getLoginUser()).thenReturn(loginUser(RbacRoleCodes.ADMIN));
        when(orgInfoMapper.queryOrgInfo("100")).thenReturn(null);
