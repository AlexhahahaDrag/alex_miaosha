package com.alex.finance.redemptionRecordInfo.controller;
...
@RequestMapping("/api/v1/redemption-record-info")
public class RedemptionRecordInfoController {
     ...
     * @param pageNum 页码
     * @param pageSize 每页大小
     * @param redemptionRecordInfoVo 查询条件
     * @return 分页结果
     */
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(value = "查询条件", name = "redemptionRecordInfoVo", dataTypeClass = RedemptionRecordInfoVo.class)}
    )
    public Result<Page<RedemptionRecordInfoVo>> getPage(@RequestParam(value = "pageNum", required = false, defaultValue = "1") Long pageNum,
                                                 @RequestParam(value = "pageSize", required = false, defaultValue = "10") Long pageSize,
                                                 @RequestBody(required = false) RedemptionRecordInfoVo redemptionRecordInfoVo) {
        return Result.success(redemptionRecordInfoService.getPage(pageNum, pageSize, redemptionRecordInfoVo));
    }
    ...
    public Result<RedemptionRecordInfoVo> getById(@PathVariable("id") Long id) {
        return Result.success(redemptionRecordInfoService.queryRedemptionRecordInfo(id));
    }

    /**
     * 新增消费券核销记录表 (按数量核销)
     * 
     * @param redemptionRecordInfoVo 消费券核销记录表 (按数量核销)信息
     * @return 操作结果
     */
    ...
    @PostMapping
    public Result<Boolean> add(@Validated({Insert.class}) @RequestBody RedemptionRecordInfoVo redemptionRecordInfoVo) {
        return Result.success(redemptionRecordInfoService.addRedemptionRecordInfo(redemptionRecordInfoVo));
    }
    ...
    @PutMapping
    public Result<Boolean> update(@Validated({Update.class}) @RequestBody RedemptionRecordInfoVo redemptionRecordInfoVo) {
        return Result.success(redemptionRecordInfoService.updateRedemptionRecordInfo(redemptionRecordInfoVo));
    }
    ...
    public Result<Boolean> delete(@RequestParam String ids) {
        return Result.success(redemptionRecordInfoService.deleteRedemptionRecordInfo(ids));
    }
}
