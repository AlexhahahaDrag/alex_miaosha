...
- 后端项目采用微服务和多模块架构，主要包含：`alex_miaosha_user`（用户服务，含 `user_boot` 与 `user_api`）、`alex_miaosha_common`（公共基础底座，含 `common_api` 纯契约层与 `common_core` 运行时核心层）、`alex_miaosha_oss`（OSS 服务）、`alex_miaosha_finance`（财务/礼尚往来微服务，含 `finance_boot` 与 `finance_api`，其中 gift 业务按 `record/person/event` 三个子领域 + `analysis` 聚合分包，对应三张表 `gift_record_info_t/gift_person_info_t/gift_event_info_t`；原 `relation` 子域与 `gift_relation_info_t` 表已于 2026-08-04 下线，关系语义由 `gift_person_info_t.relation_type` + `gift_person_relation_option_t` 词典承载）。
